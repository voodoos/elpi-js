...
let sendCallbackOrder ?(b = true) (payload : 'a Js_of_ocaml.Js.t) uuid =
...
let parse_and_compile files check =
  ...
  let compiled_prog = Elpi.API.Compile.program ~elpi [ parsed_prog ] in
...
let load files check = prog := Some (parse_and_compile files check)
...
let query_loop q more each =
  ...
  Elpi.API.Execute.loop executable ~more ~pp:each
